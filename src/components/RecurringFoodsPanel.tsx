...
export function RecurringFoodsPanel({ entries, limit = 8 }: Props) {
  const foodEntries = entries.filter((e) => (e.fluids_ml ?? 0) === 0 && e.meal_type !== 'Drink');

  const groups = new Map<string, { key: string; count: number; items: string[]; calories: number }>();
  for (const entry of foodEntries) {
    const key = normalizeMealKey(entry.items, entry.estimated_calories);
    if (!groups.has(key)) {
      groups.set(key, { key, count: 0, items: entry.items, calories: entry.estimated_calories });
    }
          ...
          {recurring.map((g) => (
            ...
            >
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">{g.calories} kcal</p>
              </div>
